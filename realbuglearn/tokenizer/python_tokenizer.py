...
class PythonTokenizer(cubert_tokenizer.CuBertTokenizer):
  ...
  def tokenize_and_abstract(
    ...
    num_char, line_char_mapping = 0, dict()
    lines = source_code.split('\n')
    for i, line in enumerate(lines):
        line_char_mapping[i+1] = num_char
        num_char += len(line) + 1
    line_char_mapping[len(lines)+1] = num_char
    ...
    for token_tuple in token_tuples:
      ...
      start_char = line_char_mapping[start_line] + start_column
      end_char = line_char_mapping[end_line] + end_column
      ...
      agnostic_tokens.append(
          unified_tokenizer.AbstractToken(
              ...
              metadata=unified_tokenizer.TokenMetadata(
                  ...
                  start=unified_tokenizer.Position(
                      line=start_line - 1, column=start_column, char=start_char),
                  end=unified_tokenizer.Position(
                      line=end_line - 1, column=end_column, char=end_char))))
      # print(agnostic_tokens[-1].spelling, source_code[start_char:end_char])
